	...
	MiscLib::Vector< std::pair< RefCountPtr< PrimitiveShape >, size_t > > *shapes)
{
	...
	ScorePrimitiveShapeVisitor< FlatNormalThreshPointCompatibilityFunc,	ImmediateOctreeType > 
		subsetScoreVisitor(m_options.m_epsilon,	m_options.m_normalThresh);
	ScorePrimitiveShapeVisitor< FlatNormalThreshPointCompatibilityFunc,	IndexedOctreeType > 
		globalScoreVisitor(3 * m_options.m_epsilon,	m_options.m_normalThresh);
	...
	do
	{
		MiscLib::Vector< std::pair< float, size_t > > sampleLevelScores(sampleLevelProbability.size());
		...
		for (size_t i = 1; i < sampleLevelProbSum.size() - 1; ++i)
			sampleLevelProbSum[i] = sampleLevelProbability[i] +	sampleLevelProbSum[i - 1];
			...
			&bestCandidateFailureProbability))
		{
			...
			// best candidate is ok!// remove the points
			...
			// update drawn candidates to reflect removal of points		// get the percentage of candidates that are invalid
